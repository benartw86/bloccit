...
RSpec.describe PostsController, type: :controller do  #RSpec created a test for PostsController. type: :controller tells RSpec to treat the test as a controller test
  ...
  let(:my_topic) { Topic.create!(name: RandomData.random_sentence, description: RandomData.random_paragraph) }  #because posts will be nested under topic, create a parent topic
  let(:my_post) { my_topic.posts.create!(title: RandomData.random_sentence, body: RandomData.random_paragraph, user: my_user) }  #update how we create my_post so that it will belong to my_topic

  # #5  contexts organize tests based on the state of an object
 ...
 # #8
  ...
  # no need for index view for posts since they will display in show view of their parent topic
  
  #we wrap specs below context so they become our signed in user specs
 ...
 # #16 we pass {id: my_post.id} to show as a parameter. These parameters are passed to the  params hash.  
        get :show, topic_id: my_topic.id, id: my_post.id  #Post routes will now include the topic_id of the parent topic, to see post in topic
        expect(response).to have_http_status(:success)
      end
 ...
 # #18  we expect the post to equal my_post because we call show with the id of  my_post. We are testing that the post returned to us is the post we asked for.
    ...
    describe "GET new" do
      it "returns http success" do    
        get :new, topic_id: my_topic.id  #get new request not includes id of the parent topic
        expect(response).to have_http_status(:success)
      end  
